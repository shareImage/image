...
1.  然后LVS，根据负载策略挑选众多 RS中的一个，然后将这个网络包的MAC地址修改成这个选中的RS的MAC
1.  然后丢给Director，Director将这个包丢给选中的RS
1.  选中的RS看到MAC地址是自己的、dip也是自己的，愉快地收下并处理、回复
1.  回复包(sip 200.200.200.1， dip 200.200.200.2)
1.  经过交换机直接回复给client了(不再走LVS)
...
1.  请求包到达LVS(图中Director)，LVS修改请求包为(sip 200.200.200.2， dip rip)
1.  请求包到达rs， rs回复(sip rip，dip 200.200.200.2)
1.  这个回复包不能直接给client，因为rip不是VIP会被reset掉(client看到的连接是vip，突然来一个rip就reset)
1.  但是因为lvs是网关，所以这个回复包先走到网关，网关有机会修改sip
1.  网关修改sip为VIP，修改后的回复包(sip 200.200.200.1，dip 200.200.200.2)发给client
...
1.  请求包到达lvs，lvs修改请求包为(vip，rip)，并将cip放入TCP Option中
1.  请求包根据ip路由到达rs， ctk模块读取TCP Option中的cip
1.  回复包(RIP, vip)被ctk模块截获，并将回复包改写为(vip, cip)
1.  因为回复包的目的地址是cip所以不需要经过lvs，可以直接发给client
...
1.  新的IP包的目的IP是某一RS的IP，然后转发给RS
1.  RS收到报文后IPIP内核模块解封装，取出用户的请求报文
1.  发现目的IP是VIP，而自己的tunl0网卡上配置了这个IP，从而愉快地处理请求并将结果直接发送给客户
...
![image.png](https://cdn.jsdelivr.net/gh/plantegg/plantegg.github.io@_md2zhihu_blog_cee8f3b4/就是要你懂负载均衡--lvs和转发模式/6021c1f2dafa0e47d437d486f13c243a.png)
